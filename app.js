...
const issuerDid =
  "did:ion:EiDCIBz8dYsrzj--lWDFZt24miMeR6r8fw42mGfr2sD1tw?-ion-initial-state=eyJkZWx0YV9oYXNoIjoiRWlEUE51RF9LbUhlcy1vQnA3UUo0aklld0ppeDd2TERHVXR3VGRMcWZWakkzUSIsInJlY292ZXJ5X2NvbW1pdG1lbnQiOiJFaUFqbEV3czJpOHlDNXctTTlyWUdlUHRWZ0VJYmNObEVnSUZMd2FiMVd4aURBIn0.";
...
app.get("/presentation-request", async (req, res) => {
  ...
  const nonce = base64url.encode(
    Buffer.from(secureRandom.randomUint8Array(10))
  );
  ...
  var requestUri = encodeURIComponent(
    `https://${req.hostname}/presentation-request.jwt?id=${req.session.id}`
  );
...
app.post("/presentation-response", parser, async (req, res) => {
  ...
  var verifiedCredential =
    validationResult.validationResult.verifiableCredentials[credentialType]
      .decodedToken;
  console.log(
    `${verifiedCredential.vc.credentialSubject.firstName} ${verifiedCredential.vc.credentialSubject.lastName} is a Verified Credential Ninja!`
  );
...
app.get("/presentation-response", async (req, res) => {
  ...
  if (req.session.verifiedCredential) {
    ...
    return res.send(
      `Congratulations, ${presentedCredential.vc.credentialSubject.firstName} ${presentedCredential.vc.credentialSubject.lastName} is a Verified Credential Ninja!`
    );
  }
